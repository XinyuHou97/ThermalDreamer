...
def read_video_frames_from_folder(folder_path, process_length, target_fps, max_res, dataset="open"):
    ...
    resized_images = [np.array(PIL.Image.fromarray((img * 255).astype(np.uint8)).resize((width, height))).astype("float32") / 255.0 for img in images]

    fps = target_fps if target_fps != -1 else 30  # Assume a default fps if not specified
    stride = max(round(30 / fps), 1)  # Assuming original fps is 30
    frames_idx = list(range(0, len(resized_images), stride))
    
    if process_length != -1 and process_length < len(frames_idx):
        frames_idx = frames_idx[:process_length]
    
    frames = [resized_images[i] for i in frames_idx]
