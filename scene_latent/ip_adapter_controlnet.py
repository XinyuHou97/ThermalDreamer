...
for i, seq in enumerate(seqs):
    startimg=Image.open("/mnt/nas/T2R_gen/firstframe-short/"+seq+".png")
    depths=np.load('/mnt/nas/T2R_gen/depth/'+seq)
    # depths=depths['depth']
    ...
    for depth in depths:
        ...
        images = ip_model.generate(pil_image=startimg, prompt=prompt, image=Image_depth, num_samples=1, num_inference_steps=50, seed=42)
        gen_imgs.append(images[0])
    
    
    # ims=[i[0] for i in gen_imgs]
    
    output_path = "/mnt/nas/T2R_gen/IP_adaptor/"+seq+".gif"
