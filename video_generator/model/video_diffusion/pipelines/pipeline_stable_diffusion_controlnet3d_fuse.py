    ...
    def __call__(
        ...
        if fix_first_frame and first_frame_output is None and first_frame_output_latent is None:
            first_frame_output = self.__call__(
                ...
                controlnet_hint2=controlnet_hint2[:,:,0,:,:] if not in_domain else controlnet_hint2[:,:,0:1,:,:],
                canny_control_ratio=canny_control_ratio,
                # b c f h w
        ...
        if first_frame_output is not None and first_frame_output_latent is None:
        ...
        with self.progress_bar(total=num_inference_steps) as progress_bar:
            ...
            if canny_control_ratio == None:
                canny_control_ratio = 0.8
                print('CANNY NOW {}'.format(canny_control_ratio))
            else:
                print('CANNY PREDICTED {}'.format(canny_control_ratio))
            for i, t in enumerate(timesteps):
                ...
                if controlnet_hint is not None:
                    ...
                    if controlnet_hint2 != None:
                        print('Using 2 control hints!')
                        down_block_res_samples2, mid_block_res_sample2 = self.controlnet(
                            latent_model_input,
                            t,
                            encoder_hidden_states=text_embeddings,
                            controlnet_cond=controlnet_hint2,
                            return_dict=False,
                        )
                        down_block_res_samples2 = [
                        down_block_res_sample * controlnet_conditioning_scale*canny_control_ratio
                        for down_block_res_sample in down_block_res_samples2
                        ]
                        mid_block_res_sample2 *= (controlnet_conditioning_scale*canny_control_ratio)
                        
                        down_block_res_samples = [down_block_res_sample2 + down_block_res_sample1*(1-canny_control_ratio) for down_block_res_sample1, down_block_res_sample2 in zip(down_block_res_samples, down_block_res_samples2)]
                        
                        # down_block_res_samples = down_block_res_samples2
                        # mid_block_res_sample = mid_block_res_sample2
                        
                            
                        # down_block_res_samples = down_block_res_samples/2 + down_block_res_samples2
                        mid_block_res_sample = mid_block_res_sample*(1-canny_control_ratio) + mid_block_res_sample2
                ...
                if video_scale > 0 and controlnet_hint is not None:
                    ...
                    control_maps_single_frame = rearrange(controlnet_hint2, 'b c f h w -> (b f) c h w')
