...
from model.video_diffusion.pipelines.pipeline_stable_diffusion_controlnet3d_fuse import Controlnet3DStableDiffusionPipeline
...
parser.add_argument('--num_sample_frames', type=int, default=10, help='total frames to inference')
...
parser.add_argument('--depth_video',type=str, default='1')
parser.add_argument('--start', type=int, default=0, help='ouput width')
parser.add_argument('--end', type=int, default=9, help='ouput width')

parser.add_argument('--p', type=str, default='both', help='ouput width')
parser.add_argument('--dmodel', type=str, default='old', help='ouput width')
parser.add_argument('--extraname', type=str, default='', help='ouput width')
parser.add_argument('--ratio', type=float, default=0.8, help='video smoothness scale')
...
if args.dmodel=='new':
    pipeline_model_path = 'Realistic_Vision_V6.0_B1_noVAE'
...
savefolder='/mnt/nas/xinyu/control-a-video/fuse/'+str(args.ratio)+'/'

if not os.path.exists(savefolder):
    os.makedirs(savefolder)
    
    
def check_control_map_redundancy(control_maps, threshold=1e-4):
    """
    检查 control_maps 每一帧是否与第一帧重复或几乎相同。

    参数:
        control_maps: Tensor，形状应为 [B, C, F, H, W] 或 [F, C, H, W]
        threshold: float，两帧之间平均差小于这个值就认为“重复”

    返回:
        None（直接打印结果）
    """
    if control_maps.dim() == 5:
        # Assume shape [B, C, F, H, W]
        control_maps = control_maps[0]  # 只取第一个 batch

    elif control_maps.dim() == 4:
        # Assume shape [F, C, H, W]
        pass
    else:
        raise ValueError("control_maps shape must be [B, C, F, H, W] or [F, C, H, W]")

    first_frame = control_maps[0]
    num_frames = control_maps.shape[0]
    
    print("Checking control map redundancy against first frame:")
    for i in range(num_frames):
        diff = torch.abs(control_maps[i] - first_frame).mean().item()
        redundant = "✅ SAME" if diff < threshold else "❌ DIFFERENT"
        print(f"Frame {i:02d} | Mean diff: {diff:.6f} | {redundant}")
...
for depthf in depthfiles:
    ...
    #     if depthf.startswith(pro):
    #         prompt=scene_prompts[pro]
    #         break
    
    # old prompts
    # testing_prompt = [prompt+', best quality, extremely detailed']
    ...
    check_control_map_redundancy(control_maps)
    ...
    control_maps2 = np.stack(frames)
    control_maps2 = repeat(control_maps2, 'f h w -> f c h w',c=1)
    control_maps2 = torch.from_numpy(control_maps2).div(255)  # 0~1
    
    

    control_maps2 = control_maps2.to(dtype=controlnet.dtype, device=controlnet.device)
    control_maps2 = F.interpolate(control_maps2, size=(h,w), mode='bilinear', align_corners=False)
    control_maps2 = rearrange(control_maps2, "(b f) c h w -> b c f h w", f=num_sample_frames)
    if control_maps2.shape[1] == 1:
        control_maps2 = repeat(control_maps2, 'b c f h w -> b (n c) f h w',  n=3)
    ...
    for file in files:
        path= npfile+file
        rframe = np.array(Image.open(path))

        rgb_array = np.stack([rframe]*3, axis=-1)
        rframes.append(rgb_array)
    
    rframes = torch.from_numpy(np.array(rframes)).div(255)
    rframes = rearrange(rframes, 'f h w c -> f c h w')
    ...
    for i in range(num_sample_frames//each_sample_frame):
        out1 = video_controlnet_pipe(
                ...
                controlnet_hint2=control_maps2[:,:,i*each_sample_frame-1:(i+1)*each_sample_frame-1,:,:] if i>0 else control_maps2[:,:,:each_sample_frame,:,:],
                ...
                canny_control_ratio = args.ratio,
